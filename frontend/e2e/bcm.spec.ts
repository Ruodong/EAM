...
test.describe('Business Capability Mapping (/app-management/bcm)', () => {
  ...
  test('search by App Name filters rows', async ({ page }) => {
    ...
    const firstAppName = (await page.locator('table tbody tr:first-child td:nth-child(2)').textContent())!.trim();
  ...
  test('reset button clears filters', async ({ page }) => {
    await page.getByPlaceholder('Application name').fill('SAP');
    await page.getByRole('button', { name: 'Search' }).click();
    await page.waitForSelector('table tbody tr', { timeout: 5000 });
  ...
  test('clicking App ID opens detail drawer', async ({ page }) => {
    ...
    const appIdLink = page.locator('table tbody tr:first-child button.text-primary-blue').first();
