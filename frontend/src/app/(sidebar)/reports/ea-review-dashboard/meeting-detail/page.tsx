...
function DashboardMeetingDetailContent() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['dashboardMeeting', meetingNo],
  ...
  const { data: decks = [] } = useQuery({
    queryKey: ['dashboardMeetingDecks', data?.id],
  ...
  const { data: actionsData } = useQuery({
    queryKey: ['dashboardMeetingActions', data?.requestId],
    queryFn: () => api.get<any>('/actions', { requestId: data?.requestId, page: 1, pageSize: 50 }),
  ...
  const actionColumns: Column<any>[] = [
    { key: 'actionId', title: 'Action ID', render: (v) => (
      <Link href={`/reports/ea-review-dashboard/action-detail?actionNo=${v}`} className="text-primary-blue hover:underline">{v}</Link>
    )},
    ...
    <div className="p-6">
      ...
      <Section title="General Data">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
          ...
          <div>
            <GDRow label="Meeting Title" value={data.title || data.meetingTitle} bold />
      ...
      <Section title="Attendees" badge={attendeesList.length}>
        {data.presenter && (
          <GDRow label="Presenter(s)" value={Array.isArray(data.presenter) ? data.presenter.join(', ') : data.presenter} bold />
        )}
        ...
        {data.emailCC && (
          <GDRow label="Email CC" value={Array.isArray(data.emailCC) ? data.emailCC.join(', ') : data.emailCC} />
        )}
      </Section>
